...
function App() {
  ...
  return <main className="min-h-screen bg-[#f4f7f6]">
    <header className="border-b border-[#dbe5e1] bg-white"><div className="mx-auto flex max-w-[1400px] items-center justify-between px-4 py-4 sm:px-8">
      <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#166b5a] text-white"><Navigation size={20} /></div><div><p className="m-0 font-['Space_Grotesk'] text-lg font-bold tracking-tight text-[#173b35]">Campus Compass</p><p className="m-0 text-xs text-[#6a7d78]">Student location registry</p></div></div>
      <Badge bg="light" className="border border-[#dbe5e1] px-3 py-2 text-[#46655c]"><span className="mr-1 inline-block h-2 w-2 rounded-full bg-[#36a269]" /> Live registry</Badge>
    </div></header>
    <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-8 lg:py-12">
      <section className="mb-8 max-w-2xl"><p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#d46b42]">Location intelligence</p><h1 className="m-0 font-['Space_Grotesk'] text-4xl font-bold tracking-tight text-[#173b35] sm:text-5xl">Know where your students are.</h1><p className="mt-4 text-base leading-7 text-[#637670]">Register student information, pinpoint their address, and keep your campus directory in one clear view.</p></section>
      {status.message && <Alert variant={status.type} dismissible onClose={() => setStatus({ type: '', message: '' })}>{status.message}</Alert>}
      <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
        <Card className="border-0 shadow-[0_14px_40px_rgba(28,65,57,0.08)]"><Card.Body className="p-5 sm:p-6"><div className="mb-5 flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#e7f3ee] text-[#166b5a]"><Plus size={19} /></div><div><h2 className="m-0 font-['Space_Grotesk'] text-xl font-bold text-[#173b35]">Register a student</h2><p className="m-0 text-sm text-[#72837e]">All fields are required</p></div></div>
